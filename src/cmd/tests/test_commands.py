...
class Test_Creator_Function_With_Mutiple_Outputs(unittest.TestCase):
    ...
    class Test_Data:
        obj:Integer_Owner
        message:Test_Creator_Function_With_Mutiple_Outputs.Message

    @dataclasses.dataclass
    class Write_Value_Message(Command):
        data:Test_Creator_Function_With_Mutiple_Outputs.Test_Data
        prev_text:str = dataclasses.field(init=False)
        new_text:str = dataclasses.field(init=False)
        def run(self)->None: 
            self.prev_text = self.data.message.text
            self.data.message.text = f"Value set to {self.data.obj.i}."
    ...
    def test_creator_function_can_produce_multiple_commands(self):
        controller =Controller()
        obj = Integer_Owner(i=0)
        composed_command = Composed_Increment()
        message_A = self.Message("Nothing has happened yet.")
        message_B = self.Message("Nothing has happened yet.")

        def update_messages(data:IncrementIntData)->Tuple[Command,...]:
            cmd_A = self.Write_Value_Message(self.Test_Data(obj, message_A))
            cmd_B = self.Write_Value_Message(self.Test_Data(obj,message_B))
            return cmd_A, cmd_B
        
        composed_command.add('test',update_messages,'post')
        controller.run(composed_command(IncrementIntData(obj,step=3)))
        
        self.assertEqual(message_A.text, "Value set to 3.")
        self.assertEqual(message_B.text, "Value set to 3.")

        controller.undo()
        self.assertEqual(message_A.text, "Nothing has happened yet.")
        self.assertEqual(message_B.text, "Nothing has happened yet.")
