...
class Editor_Tk(EditorUI):
    ...
    def __init__(
        ...
        super().__init__(
            editor, self.__item_menu, self.__item_window, self.__caseview, lang=lang
        )
    ...
    def _compose(self) -> None:  # pragma: no cover
        ...
        self.__caseview.widget.bind(
            "<Double-Button-1>", self.__double_left_click_action
        )
        ...
        self.__caseview.widget.bind(
            "<Delete>", self.__caseview.do_on_tree_item(self.delete_item)
        )
        self.__caseview.widget.bind(
            "<Control-c>", lambda e: self.__editor.copy_selection()
        )
        self.__caseview.widget.bind(
            "<Control-d>", lambda e: self.__editor.duplicate_selection()
        )
        self.__caseview.widget.bind(
            "<Control-v>", lambda e: self.__editor.paste_under_selection()
        )
        self.__caseview.widget.bind(
            "<Control-x>", lambda e: self.__editor.cut_selection()
        )
        self.__caseview.widget.bind(
            "<Control-g>", lambda e: self.__editor.group_selection()
        )
        self.__caseview.widget.bind(
            "<Control-G>", lambda e: self.__editor.ungroup_selection()
        )
        self.__caseview.widget.bind(
            "<Control-s>", lambda e: self.save_selected_cases_to_xml()
        )
    ...
    def _get_xml_path(self) -> Tuple[str, str]:
        case_full_path = askopenfilename(
