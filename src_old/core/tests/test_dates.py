...
class Test_Specifying_Date_Format(unittest.TestCase):
    ...
    def test_correct_dates_with_corresponding_correct_format_string(self)->None:
        date_format = dates.DATE_FORMATS["cs_cz"]
        for date in self.correct_dates:
            self.assertEqual(
                date, 
                dates.date_to_str(dates.enter_date(date,date_format), date_format)
            )

    def test_incorrect_dates_and_correct_format_string(self)->None:
        for date in self.incorrect_dates:
            self.assertRaises(ValueError, dates.enter_date, date, dates.DATE_FORMATS['en_us'])

    def test_incorrect_format_string(self)->None:
        self.assertRaises(ValueError, dates._validate_format, "%a.%b.%Y")
        self.assertRaises(ValueError, dates._validate_format, "%m.%Y")
        self.assertRaises(ValueError, dates._validate_format, "%d.%Y")

    def test_varying_valid_date_separators_in_format_string(self)->None:
        for sep in ('.','-','_',' '):
            conv = dates._validate_format(f"%d{sep}%m{sep}%Y")

    def test_varying_invalid_date_separators_in_format_string(self)->None:
        for sep in ('..', '---', '', '\t', '       ',''):
            self.assertRaises(ValueError, dates._validate_format, f"%d{sep}%m{sep}%Y")

    def test_missing_separators_in_date_format_string_raise_error(self):
        self.assertRaises(ValueError, dates._validate_format, "%Y%m-%d")
        self.assertRaises(ValueError, dates._validate_format, "%Y.%m%d")
        self.assertRaises(ValueError, dates._validate_format, "%Y.%mb%d")
    ...
    def test_incomplete_date_format_string_raises_error(self):
        self.assertRaises(ValueError, dates._validate_format, "%Y.%m")
        self.assertRaises(ValueError, dates._validate_format, "%Y.%m.")

    def test_two_distinct_separators_are_not_allowed(self)->None:
        self.assertRaises(ValueError, dates._validate_format, "%d.%m-%Y")
        self.assertRaises(ValueError, dates._validate_format, "%m_%d.%Y")
        self.assertRaises(ValueError, dates._validate_format, "%d.%m%Y")
    
    def test_date_format_string_must_start_with_percent(self):
        self.assertRaises(ValueError, dates._validate_format, ".%d.%Y.%m")
